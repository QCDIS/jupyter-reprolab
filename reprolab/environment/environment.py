...
def get_venv_path(venv_name):
    ...
    if os.path.isabs(venv_name):
        return venv_name
    return os.path.join(os.getcwd(), venv_name)

def find_pip_in_venv(venv_path):
    """Find pip executable in the virtual environment."""
    bin_dir = "Scripts" if os.name == "nt" else "bin"
    pip_paths = [
        os.path.join(venv_path, bin_dir, "pip"),
        os.path.join(venv_path, bin_dir, "pip3"),
        shutil.which("pip"),
    ]
...
def find_python_in_venv(venv_path):
    ...
    bin_dir = "Scripts" if os.name == "nt" else "bin"
    python_paths = [
        os.path.join(venv_path, bin_dir, "python"),
        os.path.join(venv_path, bin_dir, "python3"),
        sys.executable,
    ]
...
def freeze_venv_dependencies(venv_name: str = None):
    """Freeze dependencies from a virtual environment to requirements.txt."""
    ...
    # Freeze pip packages
    pip_path = find_pip_in_venv(venv_path)
    pip_output = None
    ...
    if not pip_output:
        ...
        python_path = find_python_in_venv(venv_path)
        if python_path:
    ...
    if os.path.exists(pip_requirements_file):
        print(f"- Activate the virtual environment and run: `pip install -r {pip_requirements_file}`")

def create_new_venv(venv_name: str):
    """Create a new virtual environment with ReproLab and essential packages."""
    essential_packages = [
        "ipykernel", "boto3", "ipylab", "pandas", "numpy", "xarray", 
        "requests", "pyarrow", "nbformat", "pyyaml", "ipywidgets", 
        "jupyterlab_widgets", "reprolab"
    ]
    ...
    try:
        ...
        print(f"[✔] Installed packages: {', '.join(essential_packages)}")
    except subprocess.CalledProcessError as e:
